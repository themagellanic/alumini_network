...
exports.sendOTP = async (req, res) => {
    ...
    //return res.status(200).json({ success: true, message: 'OTP for verification has been sent to your registered college email.'})
...
exports.me = async (req, res) => {
    ...
    const user = await User.findOne({ college_id : req.decoded.college_id }).select('college_id degree cgpa student_name gender department red_flags passout_batch permission').lean();
...
exports.updateProfile = (req, res) => {
    ...
    const _b = req.body;
    console.log(_b);
    const userDataFields = ["degree","department","cgpa","alternate_email","contact_no","matric_marks","matric_board","senior_marks","senior_board","alternate_contact_no","address","city","post_code","state","country","linkedln_link"];
    
    User
        .findOne({ college_id : req.decoded.college_id })
        .select('degree department cgpa alternate_email contact_no matric_marks matric_board senior_marks senior_board alternate_contact_no address city state post_code country linkedln_link')
        .then(user=> {
